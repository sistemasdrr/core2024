using System;
using System.Collections.Generic;

namespace DRRCore.Domain.Entities.MYSQLContext;

public partial class MEmpresaB
{
    public string? EmCodigo { get; set; }

    public string? AboCodigo { get; set; }

    public string? CupCodigo { get; set; }

    public string? TramNombre { get; set; }

    public string? CupFecped { get; set; }

    public int? EmActivo { get; set; }

    public int? IdiCodigo { get; set; }

    public DateTime? EmFecinf { get; set; }

    public string? EmNombre { get; set; }

    public string? EmSiglas { get; set; }

    public string? EmDirecc { get; set; }

    public string? EmCodpos { get; set; }

    public string? EmCiudad { get; set; }

    public string? EmDistri { get; set; }

    public int? EmTipper { get; set; }

    public string? EmTelef1 { get; set; }

    public string? EmPagweb { get; set; }

    public string? EmEmail { get; set; }

    public string? EmFecest { get; set; }

    public string? EmRegtri { get; set; }

    public string? EmRegist { get; set; }

    public string? EmRegistIng { get; set; }

    public string? EmRegen { get; set; }

    public string? SitCodigo { get; set; }

    public string? PeNombre { get; set; }

    public string? PeDocide { get; set; }

    public string? CaCodigo { get; set; }

    public string? EpCargo { get; set; }

    public string? EmActivi { get; set; }

    public string? EmActiviIng { get; set; }

    public string? EmImport1 { get; set; }

    public string? EmExport1 { get; set; }

    public string? EmTipocu { get; set; }

    public string? TituCodigo { get; set; }

    public string? TlCodigo { get; set; }

    public string? TlNombre { get; set; }

    public string? EmArea { get; set; }

    public string? EmDomant { get; set; }

    public string? EmTraba1Cant { get; set; }

    public string? EmTraba1Ano { get; set; }

    public string? EmTraba2Cant { get; set; }

    public string? EmTraba2Ano { get; set; }

    public string? EmTrabaMotivo { get; set; }

    public string? EmTrabaMotivoIng { get; set; }

    public string? EmpOpcIndu { get; set; }

    public string? EmpOpcMayo { get; set; }

    public string? EmpOpcMino { get; set; }

    public string? EmpOpcComi { get; set; }

    public string? BaFecbal1 { get; set; }

    public string? BaMoneda1 { get; set; }

    public double? BaTipcam1 { get; set; }

    public double? BaCapita1 { get; set; }

    public double? BaTotpat1 { get; set; }

    public double? BaVentas1 { get; set; }

    public double? BaUtiper1 { get; set; }

    public double? BaCaptra { get; set; }

    public string? SfCodigo { get; set; }

    public string? EmSitfin { get; set; }

    public string? EmSitfinIng { get; set; }

    public string? EsCia { get; set; }

    public string? EsMonto { get; set; }

    public string? EsVence { get; set; }

    public string? EmConinf { get; set; }

    public string? EmConinfIng { get; set; }

    public string? EmCalsbs { get; set; }

    public string? EmCenrie { get; set; }

    public string? EmCenrieIng { get; set; }

    public string? EmSupban { get; set; }

    public string? EmSupbanIng { get; set; }

    public double? EmDeudaMn { get; set; }

    public double? EmDeudaMe { get; set; }

    public string? EmDeudaObs { get; set; }

    public string? EmDeudaObsIng { get; set; }

    public string? EmDeudaTc { get; set; }

    public string? CupTipinf { get; set; }

    public string? EmDeudaFecha { get; set; }

    public string? ProvComentario { get; set; }

    public string? ProvComentarioIng { get; set; }

    public string? Rep { get; set; }

    public string? Dig { get; set; }

    public string? Tra { get; set; }

    public string? Sup { get; set; }

    public string? CpCodigo { get; set; }

    public int? TotGarMn { get; set; }

    public int? TotGarMe { get; set; }

    public string? Banqueros { get; set; }

    public string? ImpAno { get; set; }

    public string? ImpMonto { get; set; }

    public string? ExpAno { get; set; }

    public string? ExpMonto { get; set; }

    public string? Balance { get; set; }

    public string? OpcionSitfin { get; set; }

    public string? EmImport1Ing { get; set; }

    public string? EmExport1Ing { get; set; }
}
